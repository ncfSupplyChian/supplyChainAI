...
print('测试集score：', rf.score(X_test, y_test))
...
precisions = cross_val_score(rf, X_train, y_train, cv=5, scoring='precision')
print('精确率：', np.mean(precisions), precisions)
recalls = cross_val_score(rf, X_train, y_train, cv=5, scoring='recall')
print('召回率：', np.mean(recalls), recalls)
# 综合评价指标（F1 measure）是精确率和召回率的调和均值（harmonic mean），或加权平均值，也称为F-measure或fF-score。
f1s = cross_val_score(rf, X_train, y_train, cv=5, scoring='f1')
print('综合评价指标：', np.mean(f1s), f1s)
roc_auc = cross_val_score(rf, X_train, y_train, cv=5, scoring='roc_auc')
...
fpr_test, tpr_test, th_test = roc_curve(y_test, probas_test[:, 1])
fpr_train, tpr_train, th_train = roc_curve(y_train, probas_train[:, 1])
...
print(ks)
plt.plot(fpr_test, test_x_axis, color='blue', label='fpr')
plt.plot(tpr_test, test_x_axis, color='red', label='tpr')
plt.legend(loc="lower right")
